...
class CheckBundleDupeDependenciesV2 : BundleRuleBase
{
    ...
    [NonSerialized]
    internal readonly Dictionary<string, Dictionary<string, List<string>>> _allIssues = new Dictionary<string, Dictionary<string, List<string>>>();
    [SerializeField]
    internal Dictionary<List<string>, List<string>> _duplicateAssetsAndParents = new Dictionary<List<string>, List<string>>();
    ...
    List<AnalyzeResult> CheckForDuplicateDependencies(AddressableAssetSettings settings)
    {
        ...
        retVal = (from issueGroup in _allIssues
                  ...
                  select new AnalyzeResult
                  {
                      resultName = ruleName + kDelimiter +
                                           ...
                                           item,
                      severity = UnityEditor.MessageType.Warning
                  }).ToList();
    ...
    {
        _duplicateAssetsAndParents.Clear();
        ...
        foreach (var entry in validGuids)
        {
            ...
            bool found = false;
            foreach (var bundleParentSetup in _duplicateAssetsAndParents.Keys)
            {
                ...
                {
                    _duplicateAssetsAndParents[bundleParentSetup].Add(assetPath);
                    found = true;
                    break;
                }
            }
            if (!found)
            {
                ...
                _duplicateAssetsAndParents.Add(assetParents, new List<string>() { assetPath });
            }
        }
    ...
    void BuildImplicitDuplicatedAssetsSet(IEnumerable<DuplicateResult> dupeResults)
    {
        foreach (var dupeResult in dupeResults)
        {
            ...
            if (!_allIssues.TryGetValue(dupeResult.Group.Name, out groupData))
            {
                groupData = new Dictionary<string, List<string>>();
                _allIssues.Add(dupeResult.Group.Name, groupData);
            }
    ...
    public override void FixIssues(AddressableAssetSettings settings)
    {
        ...
        if (_duplicateAssetsAndParents == null || _duplicateAssetsAndParents.Count == 0)
            CheckForDuplicateDependencies(settings);

        // If we have found no duplicates, return
        if (_duplicateAssetsAndParents.Count == 0)
            return;
        ...
        EditorUtility.DisplayProgressBar("Setting up De-Duplication Group...", "", 0f / _duplicateAssetsAndParents.Count);
        // Iterate through each duplicate asset
        int bundleNumber = 1;
        foreach (var entry in _duplicateAssetsAndParents)
        {
            EditorUtility.DisplayProgressBar("Setting up De-Duplication Group...", "Creating Label Group", ((float)bundleNumber) / _duplicateAssetsAndParents.Count);
            // Create a new Label
